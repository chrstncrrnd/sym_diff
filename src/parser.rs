...
const TOK_PRECENDENCE: [Token; 5] = [Token::Power, Token::Div, Token::Mult, Token::Plus, Token::Minus];

pub fn parse(lexer: Lexer) -> Result<Expr, String>{
    let toks: Vec<Token> = lexer.collect();
    return parse_at_lvl(toks, 5);
}
...
fn parse_at_lvl(toks: Vec<Token>, level: usize) -> Result<Expr, String> {
    ...
    if level == 0 {
        ...
        println!("{:?}", toks[0]);
        return match *toks.get(0).unwrap() {
    ...
    if on_lhs {
        return parse_at_lvl(lhs, level - 1);
    } else {
        ...
        {
            return match TOK_PRECENDENCE[level - 1] {
